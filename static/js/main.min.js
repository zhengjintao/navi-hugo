...
function loaded() {
    ...
    var pageUrl = decodeURI(location.href);
    if (pageUrl.indexOf("word") !== "-1") {
        var keys = pageUrl.split("?word=")[1];
        $(".search-input").val(keys);
        $(".search-word span").text(keys)
    }
}
...
function getInputInfo(e, errortips, errorindex) {
    var errortips = errortips || "您输入的信息有误，最小字段不能小于6位";
...
function getLocalData() {
    ...
    if (pageName) {
        var pageData = localStorage.getItem(pageName + "CustomData");
        if (pageData) {
            loadData = pageData;
            var data = pageData.split(",");
            for (var i = 0; i < data.length; i++) {
                var dataValue = data[i].split("|");
                $(".duty-custom-link").eq(i).text(dataValue[0]).attr("href", dataValue[1])
            }
        } else {
            loadData = getCustomData()
        }
    }
}
function openCustomize() {
    $(".duty-custom").toggleClass("duty-custom-open");
    customizeState = !customizeState
}
$("#duty-custom-btn").click(openCustomize);
$("#duty-out").click(function () {
...
var thisLink;
$(".duty-custom-link").click(function (e) {
    if (customizeState) {
        if (e.preventDefault) {
            e.preventDefault()
        } else {
...
function getCustomData() {
    var allLink = $(".duty-custom-link");
    var linkData = [];
    for (var i = 0; i < allLink.length; i++) {
        var linkText = allLink[i].innerHTML;
        var linkUrl = allLink[i].href;
        linkData.push(linkText + "|" + linkUrl)
    }
    return linkData.join()
}
function localSave() {
    var customData = getCustomData();
    var name = $("body").attr("data-page-name");
    localStorage[name + "CustomData"] = customData;
    loadData = customData;
    layer.msg("保存成功", {
        time: 1000
    }, function () {
...
$("#duty-local-save").click(localSave);
var userID = localStorage.getItem("userInfo");
$("#duty-server-save").click(function () {
    var data = {};
    if (pageName === "web") {
        data = JSON.stringify({
            "webCustomContext": getCustomData()
        })
    } else {
        if (pageName === "ui") {
            data = JSON.stringify({
                "uiCustomContext": getCustomData()
            })
        } else {
            if (pageName === "cg") {
                data = JSON.stringify({
                    "cgCustomContext": getCustomData()
                })
            } else {
                data = JSON.stringify({
                    "officeCustomContext": getCustomData()
                })
            }
        }
    }
    ...
    if (!ifLogin) {
        if (userID) {
            $.ajax({
                ...
                success: function (res) {
                    if (pageName === "web") {
                        var serverData = res.value.webCustomContext
                    } else {
                        if (pageName === "ui") {
                            var serverData = res.value.uiCustomContext
                        } else {
                            if (pageName === "cg") {
                                var serverData = res.value.cgCustomContext
                            } else {
                                if (pageName === "cg") {
                                    var serverData = res.value.cgCustomContext
                                } else {
                                    var serverData = res.value.officeCustomContext
                                }
                            }
                        }
                    }
                    if (serverData) {
                        localStorage[pageName + "CustomData"] = serverData;
                        layer.msg("数据同步成功", {
                            time: 1000
                        }, function () {
                            location.reload()
...
$("#duty-sync-data").click(syncData);
function restoreData() {
    localStorage.removeItem(pageName + "CustomData");
    if (!localStorage.getItem(pageName + "CustomData")) {
        layer.msg("数据重置成功", {
            time: 1000
        }, function () {
            location.reload()
...
$("#duty-restore-data").click(restoreData);
var iconStatus = true;
var desStatus = true;
$(".duty-tool-switch li").click(function () {
    var index = $(this).index();
    if (index === 0) {
        iconStatus = !iconStatus;
        if (iconStatus) {
            $(".duty-tool-switch li i").eq(0).removeClass("fa-toggle-off").addClass("fa-toggle-on");
            $(".duty-card-tit img").fadeIn(500)
        } else {
            $(".duty-tool-switch li i").eq(0).removeClass("fa-toggle-on").addClass("fa-toggle-off");
            $(".duty-card-tit img").fadeOut(500)
        }
    } else {
        desStatus = !desStatus;
        if (desStatus) {
            $(".duty-tool-switch li i").eq(1).removeClass("fa-toggle-off").addClass("fa-toggle-on");
            $(".duty-card-des").fadeIn(500)
        } else {
            $(".duty-tool-switch li i").eq(1).removeClass("fa-toggle-on").addClass("fa-toggle-off");
            $(".duty-card-des").fadeOut(500)
        }
    }
    localStorage.dutyToolStatus = [iconStatus, desStatus]
});
function loadToolSwitch() {
    var dutyToolStatus = localStorage.getItem("dutyToolStatus");
    if (dutyToolStatus) {
        var val = dutyToolStatus.split(",");
        if (val[0] === "false" && val[1] === "false") {
            $(".duty-tool-switch li i").removeClass("fa-toggle-on").addClass("fa-toggle-off");
            $(".duty-card-tit img").hide();
            $(".duty-card-des").hide();
            iconStatus = false;
            desStatus = false
        } else {
...
$(".top-item").hover(function () {
    ...
    if (iconIndex !== 3) {
        $(".top-item span").eq(iconIndex).show(150)
    }
}, function () {
    $(".top-item span").eq(iconIndex).hide(150)
});
function courseSearch() {
    var inputValue = $(".search-input").val();
    if (inputValue && inputValue !== " ") {
        location.href = "/utils/course-search.html?word=" + inputValue
    } else {
        layer.msg("请输入关键词", {
            time: 700,
            offset: "90px"
...
$(".showhide-search").click(courseSearch);
...
$("#reward").click(function () {
    layer.open({
        ...
        anim: 2,
        shadeClose: true,
        content: '<img src="/upload/images/2018/7/14a171207e8f2dd7.jpg" style="padding: 10px">'
    })
});
var _hmt = _hmt || [];
(function () {
            ...
            inner.on("mousedown", function (event) {
                ...
                $(document).on("mousemove", function (event) {
                    ...
                    if (oYscrollinnerTop + moveY < 0) {
                        ...
                        if (oYscrollinnerTop + moveY > H1 - H3) {
                            inner.css("top", H1 - H3);
                            O.css("top", H1 - H2)
                        } else {
...
(function () {
    ...
    !function () {
        function r(t) {
            ...
            for (var e = 0, r = this.data.length; e < r; e++) {
                ...
                65536 < n ? (i[0] = 240 | (1835008 & n) >>> 18,
                    i[1] = 128 | (258048 & n) >>> 12,
                    i[2] = 128 | (4032 & n) >>> 6,
                    i[3] = 128 | 63 & n) : 2048 < n ? (i[0] = 224 | (61440 & n) >>> 12,
                        i[1] = 128 | (4032 & n) >>> 6,
                        i[2] = 128 | 63 & n) : 128 < n ? (i[0] = 192 | (1984 & n) >>> 6,
                            i[1] = 128 | 63 & n) : i[0] = n,
                ...
                for (var e = t.getModuleCount(), r = 0, i = 0; i < e; i++) {
                    for (var n = 0; n < e; n++) {
                        for (var o = 0, a = t.isDark(i, n), s = -1; s <= 1; s++) {
                            if (!(i + s < 0 || e <= i + s)) {
                                for (var h = -1; h <= 1; h++) {
                                    n + h < 0 || e <= n + h || 0 == s && 0 == h || a == t.isDark(i + s, n + h) && o++
                                }
                            }
                        }
                        5 < o && (r += 3 + o - 5)
        ...
        w.prototype = {
            ...
            p.getRSBlocks = function (t, e) {
                ...
                for (var i = r.length / 3, n = [], o = 0; o < i; o++) {
                    for (var a = r[3 * o + 0], s = r[3 * o + 1], h = r[3 * o + 2], l = 0; l < a; l++) {
                        n.push(new p(s, h))
                    }
                }
                return n
            ...
            L) : function () {
                ...
                return e.prototype.draw = function (t) {
                    ...
                    e.prototype.round = function (t) {
                        return t ? Math.floor(1000 * t) / 1000 : t
                    }
                    ,
                    e
            }();
        ...
        function (r, o, a) {
            var i, n, s, h = Array.prototype.indexOf, l = Object.assign, u = /MicroMessenger/i.test(navigator.userAgent), c = o.documentElement.clientWidth <= 768, t = (o.images[0] || 0).src || "", e = v("site") || v("Site") || o.title, f = v("title") || v("Title") || o.title, d = v("description") || v("Description") || "", g = {
                url: location.href,
            ...
            function C(t, e) {
                ...
                if (r === a) {
                    for (var i in t) {
                        if (t.hasOwnProperty(i) && !1 === e.call(t[i], t[i], i)) {
                            break
                        }
                    }
                } else {
            ...
            r.socialShare = function (t, e) {
                ...
                ~o.readyState.indexOf("m") ? i() : "load DOMContentLoaded readystatechange".replace(/\w+/g, function (t, e) {
                    (e ? o : r)[s ? "attachEvent" : n](s + t, function () {
                        i && (e < 6 || ~o.readyState.indexOf("m")) && (i(),
                            i = 0)
                    }, !1)
                })
        }(window, document);
    new Valine({
        ...
        el: ".comment",
        avatar: "retro",
        emoticon_url: "https://img.t.sinajs.cn/t4/appstyle/expression/ext/normal/",
        emoticon_list: {
            "smile": "e3/2018new_weixioa02_org.png",
            "lovely": "09/2018new_keai_org.png"
        },
        app_id: "TJEVkzWlX2rNRh2uhmh9fJBi-gzGzoHsz",
        app_key: "L9iAOGsKBhGQlBJBHGkIETwS",
        placeholder: '来一发？(๑╹◡╹)ﾉ"""'
    })
}());
